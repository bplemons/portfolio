...
# return the html from a given url
def getpage(url):
...
def scrapePage(soup):
    ...
    for house in houses:
        price = house.find('div', class_='srp-page-price').find('span',
                                                                class_='bowEcH').text.replace('\n', '').replace(' ', '').strip()
        address = house.find('div', class_='srp-page-address').text.replace(
